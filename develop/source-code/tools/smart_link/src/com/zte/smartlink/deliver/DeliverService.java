...
public class DeliverService
{
            ...
            throws MOSException
    {
        ...
        String msgLogContent = "send msg from " + this.name + " " + msg.toString() + " exclude " + (exclude != null ?
                Arrays.toString(exclude) :
                "");
        if (msg instanceof MoActionMsg)
        {
            Logger.logger(this.getClass()).info(msgLogContent);
        }
        else
        {
            Logger.logger(this.getClass()).debug(msgLogContent);
        }
        ...
        class AddressVisitor implements Visitor<Address>
        {
            ...
            public void visit(Address addr) throws MOSException
            {
                ...
                {
                    Logger.logger(this.getClass()).warn("send msg failed!", ex);
                    lstRes.add(new Failure(new MOSException("send msg to " + addr.toString() + " failed!", ex)));
                }
            }
        }
